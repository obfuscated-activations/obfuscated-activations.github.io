...
<div class="text-lg">
		...
		<div class="mx-4 md:mx-8 max-w-4xl text-black">
			<div class="text-left">
				...
				<div class="mt-6 md:mt-8 text-base md:text-lg">
					Luke Bailey<sup>1*</sup>, Alex Serrano<sup>2*</sup>, Abhay Sheshadri<sup>3*</sup>, 
					Mikhail Seleznyov<sup>4*</sup>, Jordan Taylor<sup>5*</sup>, Erik Jenner<sup>6*</sup> <br />
					<span class="mt-2 block">Jacob Hilton<sup>7</sup>, Stephen Casper<sup>8</sup>, Carlos Guestrin<sup>1,9</sup>, Scott Emmons<sup>6</sup></span>
				</div>
	...
	<Block padding="pb-12 pt-10">
		...
		<p class="pb-4">
			Recent <em>latent-space</em> monitoring techniques have shown promise as defenses against LLM attacks.
			...
			harmful behavior <em>via inconspicuous latent states</em>?
		</p>
		<p>
			Here, we study such <em>obfuscated activations</em>. 
		...
		When activations induced by an adversarial attack fool a latent space monitor, 
		we call these <em>obfuscated activations</em>.
		<br />
		<br />
		
		<h2 class="text-xl font-semibold mb-3">Obfuscation Attacks</h2>
		<p class="pb-4">
			An obfuscation attack is an attack that aims to induce obfuscated activations by optimizing a joint objective 
			consisting of a behavioral loss term and an obfuscation loss:
		</p>
		<div class="my-6 text-center">
			min &nbsp; L<sub>beh</sub>(M(x)) + λ<sub>obf</sub> L<sub>obf</sub>(r(x))
		</div>
		<p class="pb-4">
			For example, L<sub>beh</sub> can be a cross-entropy loss designed to make the model output a harmful string. 
			For the obfuscation loss, we often use L<sub>obf</sub> = D on inputs with the concept, where 
			D is a latent space monitor that we want to fool. That is, we directly 
			attempt to fool D.
			Finally, λ<sub>obf</sub> is some real non-negative value used for balancing the two terms.
		</p>
	...
	<Block>
		...
		We consider monitors trained to detect harmful model inputs and generations. 
		We use the obfuscation attack loss to train adversarial suffixes (both embedding soft 
		prompts and token hard prompts) to jailbreak models <em>while obfuscating against the monitor</em>.
	</Block>
	...
	<Block size="max-w-4xl" class="text-base text-gray-600">
		<em><strong>Obfuscation attacks can bypass latent harmfulness monitors</strong></em>. Here, we show the performance of obfuscation attacks at 
		fooling the probe (dots and arrows) and eliciting 
		the desired model behavior of jailbreaking (bar charts).
		Left, embedding suffix results. Right, input specific hard prompt 
		results. Baseline probe recall against attack 
		without explicit obfuscation loss is generally high. 
		Recall of probe under obfuscation is 
		significantly lower (error bars for recall 
		calculated using bootstrapping). Jailbreaking rate 
		under each obfuscated attack is shown as a bar plot.
	</Block>
	<Block>
		<p class="pb-4">
			Interestingly, we see we can fool out of distribution detectors. That is, 
			we can find <em>latent activations that are in the benign acitvation distribution 
				but lead to harmful behavior.
			</em>We might suspect that such activations, because they are so similar to 
			benign ones, fool other probes:
		</p>
	</Block>
	...
	<Block size="max-w-4xl" class="text-base text-gray-600">
		...
		onceptual sketch of obfuscation against supervised probes and OOD detectors.
		Obfuscating against probes can lead to activations far away from benign ones
		that are flagged by an OOD detector. But obfuscating against an OOD detector
		forces obfucsated activations to be close to benign ones to stay within the
		detector's decision boundary, so these activations bypass probes as a side effect.
	</Block>
	<Block>
		<p>
			And we do indeed find this is the case:
		</p>
	</Block>
	<Block size="max-w-xl" padding="pb-0">
	...
	<Block size="max-w-4xl" class="text-base text-gray-600">
		<em><strong>Obfuscation attacks can transfer to other monitors</strong></em>. 
		...
		transfers to input-based ones.
	</Block>
	<Block>
		<p class="pb-4">
			We also consider adversarially training probes.  
			We let the attacker 
			...
			<em> We find that the attacker can always find new obfuscated 
			activations to fool the monitor</em>. 
		</p>
	</Block>
	...
	<Block size="max-w-4xl" class="text-base text-gray-600">
		<em><strong>Even after 70 epochs, the attacker can still
		find new obfuscated activations that fool the monitor.</strong></em> 
		We show the monitor's recall (higher is better) as attacker and defender take turns 
		(defender using an MLP probe).
	...
	<Block>
		...
		We also consider training backdoors into models that evade latent-space monitors.
		We find using an obfuscation attack loss we can train backdoors into models 
		that are hard to detect. We also find in the black-box setting, where 
		the attacker can only poison data, there are certain poisoning rates 
		and complexity of triggers that also lead to obfuscation!
	</Block>
	...
	<Block size="max-w-4xl" class="text-base text-gray-600">
		<em><strong>Backdoored inputs can be made to appear
		benign either by reducing the number of poisoned training
		datapoints (middle), or by making the trigger more complex (right).</strong></em>
		PCA visualizations show activations from 
		layer 30 and final token positions 
		of benign, harmful, 
		and harmful + backdoor trigger inputs
		to backdoored models with data poisoning (<em>a fully 
		black box attack vector</em>). All three backdoored models in this 
	...
	<Block>
		...
		Finally, we expore SAE probing on a more complex task: writing correct SQL code.
		Firstly we identfy SAE features that correspond to SQL output tokens 
		and find we can train embedding suffixes that cause the model to write 
		SQL but fool the SAE monitor:
	</Block>
	<Block size="max-w-xl" padding="pb-0">
		<img src="sae/sae_token_score_plot.png" alt="SAE token score plot" class="w-full h-full" />
	</Block>
	<Block size="max-w-4xl" class="text-base text-gray-600">
		<em><strong>Selected SQL-related features
		correctly activate on output containing SQL code, but fail to
		activate under a soft-prompt obfuscation attack.</strong></em> 
		Here, we show GemmaScope SAE per-token activation strength for
		standard vs obfuscated generations.
	</Block>
	<Block>
		But its not all bad news for latent-space methods. We find that 
		as we obfuscate the activations of a model over more layers
		to not be detected by monitors trained to recognize SQL, 
		the model's performance on an SQL benchmark decreases.
		<em>That is, while it may be possible for models 
	...
	<Block size="max-w-4xl" class="text-base text-gray-600">
		<em><strong>As we obfuscate in more layers, model
		performance decreases.</strong></em> 
		The model's performance on the SQLEval benchmark with 
